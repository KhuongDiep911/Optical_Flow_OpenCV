...
""" J.Madge 23.04.2018, 'predict.py'.

    Source adapted from:
    https://github.com/ppwwyyxx/tensorpack/blob/master/tensorpack/predict/base.py

    A modification was performed to the '_do_call_new' function of the
    'OnlinePredictor' class to reshape the input, allowing it to be passed
    into the function as a 84x336x3 images as per the requirements of LIME,
    but reshaped to 84x84x12 as per the shape of the Tensor representing the
    input to the neural network.

    All other code is used from the link above without modification.
"""
...
from tensorpack.tfutils.common import get_tensors_by_names, get_tf_version_number
...
from tensorpack.utils.utils import execute_only_once
...
class OnlinePredictor(PredictorBase):
    ...
    def _do_call_new(self, dp):
        ...
        # J.Madge 20.04.2018 Convert image to suitable Tensor format.
        # J.Madge 20.04.2018 Image represented as a tuble containing one element - a numpy array of shape
        # (?, 84, 336, 3). This shape is not suitable for use with the trained neural network and hence must be
        # converted. The desired shape is (?, 84, 84, 12).

        # J.Madge 20.04.2018 When making a standard prediction, the image has shape (84, 336, 3). This is converted to
        # the required shape by extracting the numpy array from the tuple, splitting the images into frames and stacking
        # them along the third axis.

        image = dp[0]
        if image.ndim == 4:
            # J.Madge 20.04.2018 When LIME uses this function to make a prediction, the image has shape (x, 84, 336, 3)
            # where x corresponds to the number of images in the batch. This is converted to the required shape by
            # extracting the numpy array from the tuple, squeezing the array before splitting the images into frames and
            # stacking them along the third axis in the same way as was done for the standard prediction.
            image = np.squeeze(image, axis=0)

        dp = (frames_stack(frames(image))[None, :, :, :],)

        ret = self._callable(*dp)

        # J.Madge 20.04.2018 Extract numpy array containing predictions from the list and return.
        return ret[0]
